...
program.option("-r, --replace", "replace original files");
...
function fixSdp(sdp_string) {
    ...
    if (!inserted) console.error("invalid sdp file");
...
function fixFile(file) {
    ...
    if (fixed) {
        if (program.replace) fs.writeFileSync(file, fixed);
        else fs.writeFileSync(fname + "_fixed.sdp", fixed);

        console.log("Fixed: " + file);
    }
}

if (filestats.isFile()) fixFile(location);

if (filestats.isDirectory()) {
    const watcher = chokidar.watch(location).on("add", path => {
        fixFile(path);
    });

    if (!program.replace)
        watcher.on("change", path => {
            if (!path.endsWith("_fixed.sdp")) fixFile(path);
        });
}
